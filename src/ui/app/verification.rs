        ...
        if let (Some(model_idx), Some(quant_idx)) = (model_selected, quant_selected) {
            if model_idx < models.len() && quant_idx < quant_groups.len() {
                ...
                if !complete_downloads.contains_key(&quant.filename) {
                    *self.status.write().unwrap() = format!("File {} is not marked as downloaded", quant.filename);
                    return;
                }
                ...
                let metadata = match complete_downloads.get(&quant.filename) {
                    Some(m) => m,
                    None => {
                        *self.status.write().unwrap() = format!("Could not find metadata for {}", quant.filename);
                        return;
                    }
                };
                ...
                let expected_hash = match &metadata.expected_sha256 {
                    ...
                    None => {
                        *self.status.write().unwrap() = format!("No SHA256 hash available for {}, cannot verify", quant.filename);
                        return;
                    }
                };
                ...
                if !local_path.exists() {
                    *self.status.write().unwrap() = format!("File not found: {}", local_path.display());
                    *self.error.write().unwrap() = Some(format!("File marked as downloaded but not found at {}", local_path.display()));
                    return;
                }
                ...
                *self.status.write().unwrap() = format!("Queued {} for verification", quant.filename);
            }
        }
    }
}
